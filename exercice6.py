...
def effectuerRotation(nombreComplexe, angle_rotation, trouverModule):
    ...
    print(f"Le module est {module:.3f} et l'angle est {angle:.3f}")
    # TODO: Calculer le nouveau nombre complexe après rotation, assigner le nouveau nombre complexe à la variable 'resultat'
    Nb_Complexe_Real = nombreComplexe.real * math.cos(math.radians(angle_rotation))
    Nb_Complexe_Image = nombreComplexe.imag * math.sin(math.radians(angle_rotation))
    ...
    print(f"Le module est {nouveauModule:.3f} et l'angle est {float(nouvelAngle):.3f}")
    return resultat
